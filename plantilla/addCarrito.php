<?PHP
	session_start();
	if(!isset($_SESSION['carrito']) || count($_SESSION['carrito']) <= 0){ /*sino esta establecido o la cuenta es menor que 0 creamos el carritovacio*/
		$_SESSION['carrito'] = [];
	}
	...
	$talla = null;
	$color = null;
	if(isset($_POST['id_producto']) && $_POST['id_producto'] != ''){ /*mientras no este vacia y esta establecida genera el codproducto*/
	...
	if(!isset($_SESSION['carrito'][$id_producto])){ /*variable de sssecion carrito tiene dentro el variable de id_producto y se inicializa en 0*/
