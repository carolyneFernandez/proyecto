	...
	if(isset($_SESSION['carrito']) && count($_SESSION['carrito']) > 0){  /*compueba si esta definida carrito y contar la cantidad de elementos que tiene array carrito*/
       ...
       /*codusuario(que se tiene desde la variable sesion )*/
			...
			/*query  para el codpedido ,elija el mayor codpedido con los datos */
				...
				foreach($_SESSION['carrito'] as $id => $objeto){ /*pasarlo como base la variable session carrito por un lado id y objeto*/
					$connection->query("INSERT INTO incluyen VALUES('".$id."', '".$fila['ID']."', '".$objeto['cantidad']."')"); /*insertar inclyen */
					$connection->query("UPDATE producto SET stock=stock - ".$objeto['cantidad']." WHERE codproducto = '".$id."'"); /*restar cantidad del producto*/
